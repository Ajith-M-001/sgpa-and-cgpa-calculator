...
const secondSemesterTag = document.querySelector('a[href="2nd_sem.html"]');
secondSemesterTag.classList.add('active');
...
const subject9Input = document.getElementById('subject9');
...
const internal9Input = document.getElementById('internal9');
const external9Input = document.getElementById('external9');
...
const subject9Error = document.getElementById('subject-error9');
...
const internal9Error = document.getElementById('internal9-error');
const external9Error = document.getElementById('external9-error');
...
function calculate(event) {
    ...
    const internal9 = parseInt(internal9Input.value)
    const external9 = parseInt(external9Input.value);
    ...
    if (isNaN(internal9) || internal9 < 0 || internal9 > 40) {
        internal9Error.textContent = 'Internal marks should be between 0 and 40.';
        internal9Error.classList.remove('hidden');
        hasErrors = true;
    } else {
        internal9Error.classList.add('hidden');
    }

    if (isNaN(external9) || external9 < 0 || external9 > 60) {
        external9Error.textContent = 'External marks should be between 0 and 60.';
        external9Error.classList.remove('hidden');
        hasErrors = true;
    } else {
        external9Error.classList.add('hidden');
    }
            ...
            switch (selectedSubject) {
                case '20MCA251':
                    if (selectedSubjects.includes('20MCA255')) {
                        errorDiv.textContent = 'You cannot select both 20MCA251 and 20MCA255.';
                        errorDiv.classList.remove('hidden');
                ...
                case '20MCA262':
                    if (selectedSubjects.includes('20MCA263')) {
                        errorDiv.textContent = 'You cannot select both 20MCA262 and 20MCA263.';
                        errorDiv.classList.remove('hidden');
                        errorDiv.classList.add('text-red-500');
                        dropdown.addEventListener('focus', () => {
                            errorDiv.classList.add('hidden');
...
internal9Input.addEventListener('change', () => {
    internal9Error.classList.add('hidden');
    internal9Input.classList.remove('border-red-500');
});
internal9Input.addEventListener('focus', () => {
    internal9Error.classList.add('hidden');
    internal9Input.classList.remove('border-red-500');
});
...
external9Input.addEventListener('change', () => {
    external9Error.classList.add('hidden');
    external9Input.classList.remove('border-red-500');
});
external9Input.addEventListener('focus', () => {
    external9Error.classList.add('hidden');
    external9Input.classList.remove('border-red-500');
});
...
// -------------------------------------------------------------
const totalInput8 = document.getElementById('total9');
const gradeInput8 = document.getElementById('grade9');
function updateTotal9() {
    const internalMarks = parseFloat(internal9Input.value) || 0;
    const externalMarks = parseFloat(external9Input.value) || 0;
    const totalMarks = internalMarks + externalMarks;

    // update total input
    totalInput8.value = totalMarks;
